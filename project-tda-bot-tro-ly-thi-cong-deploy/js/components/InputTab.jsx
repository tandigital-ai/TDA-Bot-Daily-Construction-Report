...
function InputTab({ cfg, setCfg, t, onNormalized, onGoto }) {
  const [rawText, setRawText] = useStateIn("");
  const [providerId, setProviderId] = useStateIn("");
  const [model, setModel] = useStateIn("");
  ...
  const [bootstrapped, setBootstrapped] = useStateIn(false);
  const logsRef = useRefIn(null);
  const fileInputRef = useRefIn(null);

  // Bootstrap từ IndexedDB
  useEffectIn(() => {
    (async () => {
      const [raw, lp, lm] = await Promise.all([
        window.IDB.get("raw"),
        window.IDB.get("last_provider"),
        window.IDB.get("last_model"),
      ]);
      setRawText(raw || "");
      setProviderId(lp || "");
      setModel(lm || "");
  ...
  useEffectIn(() => {
    ...
    const t = setTimeout(() => { window.IDB.set("raw", rawText).catch(() => {}); }, 400);
    return () => clearTimeout(t);
  }, [rawText, bootstrapped]);
  useEffectIn(() => { if (bootstrapped && providerId) window.IDB.set("last_provider", providerId).catch(() => {}); }, [providerId, bootstrapped]);
  useEffectIn(() => { if (bootstrapped && model) window.IDB.set("last_model", model).catch(() => {}); }, [model, bootstrapped]);
  ...
  const models = useMemoIn(() => {
    if (!providerId || !cfg?.providers) return [];
    return cfg.providers[providerId]?.models || [];
  }, [providerId, cfg]);

  useEffectIn(() => {
    if (providerId && !model && models.length > 0 && cfg?.providers?.[providerId]) {
