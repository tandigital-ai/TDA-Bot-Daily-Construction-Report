...
function ProvidersTab({ cfg, setCfg, t }) {
  ...
  const handleExport = async () => {
    // Xuất toàn bộ IndexedDB (config + rows + tab + raw ...)
    try {
      const all = await window.IDB.exportAll();
      const blob = new Blob([JSON.stringify(all, null, 2)], { type: "application/json" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `kh_tc_backup_${new Date().toISOString().slice(0,10)}.json`;
      a.click();
  ...
  const handleImport = async (e) => {
    ...
    try {
      ...
      // Nếu là backup toàn bộ (có kv, rows)
      if (imported.kv && Array.isArray(imported.kv)) {
        if (!confirm("Nhập backup toàn bộ sẽ THAY THẾ tất cả dữ liệu hiện tại (config + bảng). Tiếp tục?")) { e.target.value = ""; return; }
        await window.IDB.importAll(imported);
        alert("✅ Đã nhập backup. Trang sẽ tải lại để áp dụng.");
        setTimeout(() => location.reload(), 500);
      } else if (imported.providers) {
        // Chỉ config
        save(imported);
        alert("✅ Đã nhập cấu hình");
      } else {
        throw new Error("File không hợp lệ (không phải backup hoặc config)");
      }
    } catch (err) {
